// 由 tools/build_exercises.py 生成，内容与 data/exercises.json 一致
export const EXERCISES = [
  ...
  },
  {
    ...
    "pandasTips": "<code>.iloc[3:6]</code>：按行<b>位置</b>切片（从 0 开始、左闭右开），即第 4~6 行，对应 <code>LIMIT 3 OFFSET 3</code>。"
  },
  {
    "id": "I01",
    "level": "intermediate",
  ...
  },
  {
    ...
    "pandasTips": "<code>s.nunique()</code>：统计一列去重后的取值个数（默认忽略 NaN），对应 <code>COUNT(DISTINCT 列)</code>。"
  },
  {
    "id": "I12",
    "level": "intermediate",
    ...
    "pandasTips": "<code>s.dt.strftime('%Y-%m')</code>：把 datetime 列格式化为 <code>YYYY-MM</code> 字符串，对应 SQL 的 <code>strftime('%Y-%m', 列)</code>。<br><code>.dt</code> 是日期时间访问器，地位类似字符串的 <code>.str</code>。"
  },
  {
    "id": "I13",
    "level": "intermediate",
    ...
    "pandasTips": "字符串 Series 可以直接用 <code>+</code> 逐行拼接，如 <code>df['a'] + '-' + df['b']</code>；也可用 <code>df['a'].str.cat(df['b'], sep='-')</code>。"
  },
  {
    "id": "I14",
    "level": "intermediate",
  ...
  },
  {
    ...
    "pandasTips": "<code>~</code>：对布尔 Series 逐元素取反，<code>~s.isin(v)</code> 即「不在集合 v 中」。注意取反用 <code>~</code> 而不是 <code>not</code>。"
  },
  {
    "id": "A12",
    "level": "advanced",
    ...
    "pandasTips": "<code>df.groupby('列', group_keys=False).head(n)</code>：保留每个分组的前 n 行（按 DataFrame 当前顺序），配合先排序即可实现「每组 Top-N」。"
  },
  {
    "id": "A13",
    "level": "advanced",
    ...
    "pandasTips": "<code>s.shift(1)</code>：整列向下平移 1 行、首行补 NaN，对应 <code>LAG</code>；<code>s.shift(-1)</code> 向上平移，对应 <code>LEAD</code>。"
  },
  {
    "id": "A14",
    "level": "advanced",
    ...
    "pandasTips": "<code>df.groupby('列')['x'].transform('sum')</code>：先按组聚合，再把结果<b>广播回每一行</b>（返回值与原表等长），对应窗口函数 <code>SUM(x) OVER (PARTITION BY 列)</code>。"
  },
  {
    "id": "A15",
    "level": "advanced",
    ...
    "pandasTips": "<code>df.groupby('列')['x'].transform('mean')</code>：把「每组的平均值」对齐到每一行，之后可直接与原列逐行比较，对应窗口函数 / 相关子查询。"
  },
  {
    "id": "A16",
    "level": "advanced",
    ...
    "pandasTips": "<code>s.rolling(窗口, min_periods=1).mean()</code>：滑动窗口平均，<code>rolling(3)</code> 表示当前行加前 2 行；<code>min_periods=1</code> 让窗口不足 3 行时也照常计算，对应 <code>ROWS BETWEEN 2 PRECEDING AND CURRENT ROW</code>。"
  },
  {
    "id": "A17",
    "level": "advanced",
  ...
  },
  {
    ...
    "pandasTips": "<code>s.rank(method='min', ascending=False)</code>：降序排名，并列取共同的最小名次（之后跳号），对应 <code>RANK()</code>；<code>method='dense'</code> 对应 <code>DENSE_RANK</code>，<code>method='first'</code> 对应 <code>ROW_NUMBER</code>。"
  },
  {
    "id": "A19",
    "level": "advanced",
  ...
  },
  {
    ...
    "pandasTips": "<code>df.groupby('列')['x'].nunique()</code>：每组内的去重计数，对应 <code>COUNT(DISTINCT x)</code>。<br><code>(s &gt;= 2).astype(int)</code>：把布尔列转成 0/1，之后 <code>sum</code> 即「满足条件的行数」，对应 <code>SUM(CASE WHEN ... THEN 1 ELSE 0 END)</code>。"
  }
];
